...
class DoubleDocumentBottomSheet extends StatefulWidget {
  ...
  @override
  State<DoubleDocumentBottomSheet> createState() =>
      _DoubleDocumentBottomSheetState();
}

class _DoubleDocumentBottomSheetState extends State<DoubleDocumentBottomSheet> {
  @override
  void initState() {
    super.initState();
    getFront();
  ...
  String _formatDate(DateTime date) {
    return DateFormat('MM/dd/yyyy').format(date); // 'MM:DD:YYYY' format
  }
  ...
  _uploadImage() async {
    ...
    request.files.add(
      http.MultipartFile(
        widget.parameter2, // Field name expected by the server
        File(_imageFile2!.path).readAsBytes().asStream(),
        File(_imageFile2!.path).lengthSync(),
        filename: _imageFile2!.path,
      ),
    );
    // Add other parameters
    request.fields['d_id'] = did; // Replace with your actual ID
    if (widget.isfieldAvailable != null) {
      request.fields[widget.numberParamter ?? ''] =
          numberController.text.toString(); // Replace with your actual ID
    }

    if (widget.isDateAvaiabl != null) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          uploadedImage2 =
              jsonResponse['data'][0][widget.forInsideArray2] ?? '';
        });
      } else {
